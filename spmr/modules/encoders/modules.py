...
from .arc2face_models import CLIPTextModelWrapper as Arc2FaceCLIPTextModelWrapper
...
class GeneralConditionerCamera(GeneralConditioner):
    OUTPUT_DIM2KEYS = {2: "camera", 3: "crossattn", 4: "cond_images", 5: "cond_images"}
    KEY2CATDIM = {"vector": 1, "crossattn": 2, "concat": 1}
...
class FrozenCLIPEmbedderArc2Face(AbstractEmbModel):
    ...
    def __init__(
        ...
        self.tokenizer = CLIPTokenizer.from_pretrained(version)
        self.encoder = Arc2FaceCLIPTextModelWrapper.from_pretrained(
        ckpt_path, subfolder="encoder", torch_dtype=torch.float32
    )
        self.device = device
        self.max_length = max_length
        if freeze:
    ...
    def project_face_embs(self, face_embs):
        ...
        input_ids = self.tokenizer(
            ...
            max_length=self.max_length,
            return_tensors="pt",
        ).input_ids.to(self.device).repeat(bs, 1)

        face_embs_padded = F.pad(face_embs, (0, self.encoder.config.hidden_size - 512), "constant", 0)
        token_embs = self.encoder(input_ids=input_ids, return_token_embs=True)
        token_embs[:, input_ids[0] == arcface_token_id] = face_embs_padded[:, None]
    ...
    def forward(self, text):
        ...
        bs = len(text)
        z = torch.zeros((bs,self.max_length,self.encoder.config.hidden_size), device=self.device, dtype=torch.float32)

        zero_indices = np.array([i for i in range(len(text)) if isinstance(text[i], str)])
        non_zero_indices = np.setdiff1d( np.arange(bs), zero_indices,)
        zero_input = [text[i] for i in zero_indices]
        non_zero_input = [text[i] for i in non_zero_indices]

        if len(non_zero_input) > 0:
            z_non_zero_output = self.project_face_embs(torch.stack(non_zero_input,dim=0).to(self.device))
            z[non_zero_indices] = z_non_zero_output
        ...
        if len(zero_input) > 0:
            ...
            if self.layer == "last":
                z_zeroed_input = outputs.last_hidden_state
            elif self.layer == "pooled":
                z_zeroed_input = outputs.pooler_output[:, None, :]
            else:
                z_zeroed_input = outputs.hidden_states[self.layer_idx]

            z[zero_indices, :] = z_zeroed_input
...
class FrozenOpenCLIPImagePredictionEmbedder(AbstractEmbModel):
    def __init__(
        ...
        if ckpt_path is not None:
            ...
            print('Initiallizing OpenClipEmbedder from ', ckpt_path)
...
class CameraPoseEmbedder(AbstractEmbModel):
    def __init__(self, key='camera'):
        super().__init__()
        self.key = key
    def forward(self, c):
        c_shape = c.shape
        c = c.view(-1, *c_shape[2:]).contiguous()
        return c
