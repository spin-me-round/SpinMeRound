...
class Guider(ABC):
    ...
    def prepare_inputs(
        self, x: torch.Tensor, s: float, c: Dict, uc: Dict, **ignore_kwargs,
    ) -> Tuple[torch.Tensor, float, Dict]:
        pass


class NoGuider(Guider):
    def __init__(self, **ignore_params):
        super().__init__()

    def __call__(self, x: torch.Tensor, sigma: float) -> torch.Tensor:
        return x

    def prepare_inputs(self, x, s, c, uc, **ignore_params):
        return x, s, c

class VanillaCFG(Guider):
    def __init__(self, scale: float, **ignore_params):
        self.scale = scale
    ...
    def prepare_inputs(self, x, s, c, uc, **ignore_kwargs):
        c_out = dict()

        for k in c:
            if k in ["vector", "crossattn", "concat", "camera", "cond_images"]:
...
class VanillaCFGSpMR(Guider):
    ...
    def prepare_inputs(self, x, s, c, uc, gt_z, empty_image, picked_frames,):
        c_out = dict()

        for k in c:
            if k in ["vector", "crossattn", "concat", "camera", "cond_images"]:
                # c_out[k] = torch.cat((c[k], uc[k],), 0)
                c_out[k] = torch.cat((uc[k], c[k],), 0)
            else:
                assert c[k] == uc[k]
                c_out[k] = c[k]
        x[picked_frames] = gt_z
        x_padded = torch.cat([x] * 2)
        if empty_image is not None:
            x_padded[picked_frames] = empty_image
        return x_padded, torch.cat([s] * 2), c_out
...
class LinearPredictionGuider(Guider):
    ...
    def prepare_inputs(
        ...
        for k in c:
            if k in ["vector", "crossattn", "concat", "camera", "cond_images"] + self.additional_cond_keys:
                c_out[k] = torch.cat((uc[k], c[k]), 0)
            else:
                assert c[k] == uc[k]
                c_out[k] = c[k]
