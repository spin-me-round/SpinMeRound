...
def sample(
    ...
    print('desided_angles : ', desired_angles.transpose(1,0))
    ...
    if crop:
        ...
        cropped_img = cv2.resize(image, (size, size))
...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Accordion("Angle options", open=False):
                crop = gr.Checkbox(
                    True, label='Crop input image')
                remove_bg = gr.Checkbox(
                    True, label='Remove background')
    ...
    ips = [image_block, angle_sampling, num_steps, scale, crop, remove_bg, num_frames]
    run_button.click(
        fn=sample,
        inputs=ips,
        outputs=[gallery],
